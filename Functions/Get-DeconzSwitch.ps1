function Get-DeconzSwitch{
    ...
    PROCESS{
        ...
        if($PSBoundParameters.ContainsKey('SwitchName')){
            $Sensors = ((Invoke-WebRequest -Uri "$($FullURI)/sensors").content | ConvertFrom-Json).psobject.properties | Where-Object {$_.value.Type -eq 'ZHASwitch' -and $_.value.Name -eq "$SwitchName"}
            }
        else{
            $Sensors = ((Invoke-WebRequest -Uri "$($FullURI)/sensors").content | ConvertFrom-Json).psobject.properties | Where-Object {$_.value.Type -eq 'ZHASwitch'} | Sort-Object
        }  
